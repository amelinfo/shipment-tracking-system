...
class TrackingServiceIntegrationTest {
    ...
    @BeforeEach
    void setUp() {
        trackingService = new TrackingService(externalShipmentService, kafkaTemplate, shipmentMetrics, shipmentRepository);
    ...
    void testProcessTracking_NoStatusChange() {
        ...
        // First call - mock repository to return empty (new shipment)
        when(shipmentRepository.findByTrackingId(trackingId)).thenReturn(Optional.empty());
        
        TrackingResult firstResult = trackingService.processTracking(trackingId);
        assertTrue(firstResult.success());
        assertTrue(firstResult.statusChanged()); // First time should be considered a change
        
        // Now mock repository to return existing shipment with same status
        var existingShipment = Shipment.of(trackingId, status);
        when(shipmentRepository.findByTrackingId(trackingId)).thenReturn(Optional.of(existingShipment));
